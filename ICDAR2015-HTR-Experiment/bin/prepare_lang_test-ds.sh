...
cat ${lm} | \
    ...
    kaldi-docker arpa2fst - | \
    kaldi-docker fstprint | \
    ...
    awk -v dm=$DUMMY '
         { if (NF>=4) {if ($3=="<s>") $3="<eps>"; if ($4=="<s>") $4="<eps>";
           ...
           print }' | \
    kaldi-docker fstcompile \
    --isymbols=${testd}/words.txt \
    --osymbols=${testd}/words.txt \
    --keep_isymbols=false \
    --keep_osymbols=false | \
    kaldi-docker fstrmepsilon | \
    kaldi-docker fstarcsort --sort_type=ilabel > ${testd}/G.fst

echo
set +e
kaldi-docker fstisstochastic ${testd}/G.fst || echo "[info]: G not stochastic." 1>&2
set -e
