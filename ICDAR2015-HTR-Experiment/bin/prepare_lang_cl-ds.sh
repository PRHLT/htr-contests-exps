...
awk -v bs="$BLANK" -v ws="$WSPACE" -v dm="$DUMMY" '
    BEGIN{dflg=0; sflg=0}
    { p=$1; if (p==dm) dflg=1; if (p==ws) sflg=1; if (p!=bs && p!="<eps>") print p"\t1.0\t"p }
...
awk -v bs="$BLANK" -v ws="$WSPACE" -v dm="$DUMMY" '
    BEGIN{flg=0}
    {if ($1==dm) flg=1; if (ws!=$1 && bs!=$1 && "<eps>"!=$1) print $1}
...
mv ${srcdir}/silence_phones.txt ${dir}/phones/silence.txt
mv ${srcdir}/nonsilence_phones.txt ${dir}/phones/nonsilence.txt
...
make_lexicon_fst.pl \
    ...
    kaldi-docker fstcompile --isymbols=${dir}/phones.txt --osymbols=${dir}/words.txt \
    --keep_isymbols=false --keep_osymbols=false |   \
    kaldi-docker fstaddselfloops  \
    "echo ${phone_disambig_symbol} |" "echo ${word_disambig_symbol} |" | \
    kaldi-docker fstarcsort --sort_type=olabel > ${dir}/L_disambig.fst || exit 1;
