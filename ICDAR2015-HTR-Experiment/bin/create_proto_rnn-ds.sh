...
kaldi-docker gmm-init-mono --print-args=false ${DIROUT}/topo ${DIM} auxMdl auxTree
...
{ 
  ...
  echo -e "$i 0 ${iblnkPDF}";
  echo "</Triples>"; 
  kaldi-docker gmm-copy --print-args=false --binary=false auxMdl - 2>/dev/null |
...
kaldi-docker gmm-copy --print-args=false --binary=true ${DIROUT}/auxMDL ${DIROUT}/new.mdl #2>/dev/null
[ "$?" != 0 ] && exit 1;
...
kaldi-docker copy-tree --print-args=false --binary=true ${DIROUT}/auxTree ${DIROUT}/new.tree #2>/dev/null
[ "$?" != 0 ] && exit 1;

rm auxMdl auxTree

kaldi-docker gmm-info --print-args=false ${DIROUT}/new.mdl
kaldi-docker tree-info --print-args=false ${DIROUT}/new.tree
