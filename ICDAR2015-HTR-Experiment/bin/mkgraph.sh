...
if [[ ! -s $lang/tmp/LG.fst || $lang/tmp/LG.fst -ot $lang/G.fst || \
  ...
  kaldi-docker fsttablecompose $lang/L_disambig.fst $lang/G.fst | kaldi-docker fstdeterminizestar --use-log=true | \
    kaldi-docker fstminimizeencoded | kaldi-docker fstpushspecial | \
    kaldi-docker fstarcsort --sort_type=ilabel > $lang/tmp/LG.fst || exit 1;
  kaldi-docker fstisstochastic $lang/tmp/LG.fst || echo "[info]: LG not stochastic."
fi

clg=$lang/tmp/CLG_${N}_${P}.fst

if [[ ! -s $clg || $clg -ot $lang/tmp/LG.fst ]]; then
  kaldi-docker fstcomposecontext --context-size=$N --central-position=$P \
   --read-disambig-syms=$lang/phones/disambig.int \
   --write-disambig-syms=$lang/tmp/disambig_ilabels_${N}_${P}.int \
    $lang/tmp/ilabels_${N}_${P} < $lang/tmp/LG.fst | \
    kaldi-docker fstarcsort --sort_type=ilabel > $clg
  kaldi-docker fstisstochastic $clg  || echo "[info]: CLG not stochastic."
fi

if [[ ! -s $dir/Ha.fst || $dir/Ha.fst -ot $model  \
    || $dir/Ha.fst -ot $lang/tmp/ilabels_${N}_${P} ]]; then
  if $reverse; then
    kaldi-docker make-h-transducer --reverse=true --push_weights=true \
      --disambig-syms-out=$dir/disambig_tid.int \
      --transition-scale=$tscale $lang/tmp/ilabels_${N}_${P} $tree $model \
      > $dir/Ha.fst || exit 1;
  else
    kaldi-docker make-h-transducer --disambig-syms-out=$dir/disambig_tid.int \
      --transition-scale=$tscale $lang/tmp/ilabels_${N}_${P} $tree $model \
       > $dir/Ha.fst || exit 1;
  fi
fi
...
if [[ ! -s $dir/HCLGa.fst || $dir/HCLGa.fst -ot $dir/Ha.fst || \
  ...
  kaldi-docker fsttablecompose $dir/Ha.fst "$clg" | kaldi-docker fstdeterminizestar --use-log=true \
    | kaldi-docker fstrmsymbols $dir/disambig_tid.int | kaldi-docker fstrmepslocal | \
     kaldi-docker fstminimizeencoded > $dir/HCLGa.fst || exit 1;
  kaldi-docker fstisstochastic $dir/HCLGa.fst || echo "HCLGa is not stochastic"
fi

if [[ ! -s $dir/HCLG.fst || $dir/HCLG.fst -ot $dir/HCLGa.fst ]]; then
  kaldi-docker add-self-loops --self-loop-scale=$loopscale --reorder=true \
    $model < $dir/HCLGa.fst > $dir/HCLG.fst || exit 1;

  if [ $tscale == 1.0 -a $loopscale == 1.0 ]; then
    # No point doing this test if transition-scale not 1, as it is bound to fail.
    kaldi-docker fstisstochastic $dir/HCLG.fst || echo "[info]: final HCLG is not stochastic."
  fi
fi
...
kaldi-docker am-info --print-args=false $model | grep pdfs | awk '{print $NF}' > $dir/num_pdfs
